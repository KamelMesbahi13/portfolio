...
const HomeContactUs = () => {
  ...
  // ... (Validation Logic stays the same, removed for brevity but keep your original validation logic here)
  const validateField = (name, value) => {
    switch (name) {
      case "firstName":
      case "lastName": {
  ...
  // UPDATED: Increased text size for mobile (text-base) to prevent zooming and improve readability
  const getInputClasses = (fieldName) => {
    const hasError = errors[fieldName] && touched[fieldName];
    return `w-full px-4 py-3 text-base sm:text-sm text-gray-900 transition-all border outline-none rounded-xl bg-gray-50/50 hover:bg-gray-50 ${
      hasError
        ? "border-red-500 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 pr-8"
        : "border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20"
    }`;
  ...
  return (
    // UPDATED: Removed 'container' class, added w-full and better padding structure
    <div className="container flex items-center justify-center w-full min-h-screen px-4 py-8 sm:px-6 lg:px-8">
      <div className="w-full max-w-6xl overflow-hidden bg-white shadow-2xl rounded-2xl sm:rounded-3xl">
        <div className="flex flex-col lg:flex-row">
          {/* Left Panel - Info */}
          {/* UPDATED: Added min-height so it doesn't look squashed */}
          <div
            className="w-full lg:w-[40%] p-6 sm:p-8 lg:p-12 relative flex flex-col justify-between overflow-hidden min-h-[300px]"
            style={{ backgroundColor: colors.main }}
          >
            <div className="absolute top-0 left-0 w-full h-full pointer-events-none bg-gradient-to-br from-white/10 to-transparent" />
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6 text-white md:mb-10">
                <div className="flex items-center justify-center w-8 h-8 border rounded-full sm:w-10 sm:h-10 bg-white/10 backdrop-blur-md border-white/20">
                  <div className="w-4 h-4 bg-white rounded-full sm:w-5 sm:h-5 shadow-glow" />
                </div>
                <span className="text-base font-semibold tracking-wide sm:text-lg text-white/90">
                  Kamel Mesbahi
                </span>
              </div>
              ...
              <motion.h2
                ...
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6 }}
                className="mb-4 text-2xl font-medium leading-tight text-white sm:mb-6 sm:text-3xl lg:text-4xl"
              >
                Turn Ideas Into <br />
                <span style={{ color: colors.second }}>Digital Reality.</span>
              </motion.h2>
              ...
              <motion.p
                ...
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, delay: 0.1 }}
                className="block max-w-md text-sm leading-relaxed text-gray-300/90"
              >
                Whether it's a website, web app, or social media campaign, I
                craft modern, high-performing digital experiences that make your
                brand stand out.
            ...
            {/* UPDATED: Removed 'hidden' on mobile, resized to fit better */}
            <div className="relative z-10 flex-col items-center hidden w-full mt-8 lg:flex">
              <div className="relative w-full bg-white rounded-2xl p-6 shadow-xl overflow-hidden min-h-[140px] flex flex-col justify-center group">
                <Quote
                  className="absolute w-16 h-16 text-gray-100 transition-transform duration-700 transform opacity-50 pointer-events-none -top-2 -right-2 rotate-12 group-hover:scale-110"
                  fill="currentColor"
                />
                <div className="absolute top-6 left-0 w-1.5 h-8 rounded-r-full bg-orange-500" />
                <AnimatePresence mode="wait">
                  <motion.div
                    key={currentQuote}
                    initial={{ opacity: 0, x: 10 }}
                  ...
                  >
                    <p className="mb-4 text-sm italic font-medium leading-relaxed text-gray-800 line-clamp-3">
                      "{quotes[currentQuote].text}"
                    </p>
                    <div className="flex items-center gap-2">
          ...
          {/* Right Panel - Form */}
          <div className="w-full lg:w-[60%] bg-white p-6 sm:p-8 lg:p-12">
            {/* Success/Error Messages */}
            <AnimatePresence>
              {submitStatus && (
                <motion.div
            ...
            >
              ...
              <div className="space-y-1.5 relative">
                ...
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setIsOpen(!isOpen)}
                    className={`w-full px-4 py-3 text-left bg-gray-50/50 border rounded-xl flex items-center justify-between transition-all outline-none hover:bg-gray-50
                      ${
                        errors.service && touched.service
                  ...
                  <AnimatePresence>
                    {isOpen && (
                      <motion.ul
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className="absolute z-50 w-full mt-2 overflow-hidden overflow-y-auto bg-white border border-gray-100 shadow-xl rounded-xl max-h-60"
                      >
                        {services.map((service) => (
                          <li
                            key={service}
                            onClick={() => handleServiceSelect(service)}
                            className="flex items-center justify-between px-4 py-3 text-sm text-gray-700 cursor-pointer hover:bg-gray-50 hover:text-orange-500"
                          >
                            <span>{service}</span>
                            {formData.service === service && (
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <textarea
                    ...
                    className={`w-full px-4 py-3 text-base sm:text-sm text-gray-900 transition-all border outline-none resize-none rounded-xl bg-gray-50/50 hover:bg-gray-50 ${
                      errors.message && touched.message
              ...
              <motion.button
                ...
                whileHover={{ scale: isSubmitting ? 1 : 1.01 }}
                whileTap={{ scale: isSubmitting ? 1 : 0.98 }}
                className="flex items-center justify-center w-full gap-2 py-4 text-sm font-bold text-white transition-all shadow-lg rounded-xl hover:shadow-xl hover:shadow-orange-500/20 disabled:opacity-70"
                style={{ backgroundColor: colors.main }}
              >
                {isSubmitting ? (
                  <>
