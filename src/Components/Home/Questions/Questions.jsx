      ...
      <Accordion.Item value={item.id} className="mb-4">
        <div
          className={`
            ...
            ${
              isOpen
                ? "border-mainColor/35 bg-gradient-to-br from-mainColor/[0.08] to-mainColor/[0.03] shadow-md dark:border-white/15 dark:bg-white/[0.08] dark:shadow-none dark:from-transparent dark:to-transparent"
                : "border-mainColor/20 bg-white shadow-sm hover:shadow-md hover:border-mainColor/30 dark:border-white/5 dark:bg-white/[0.03] dark:shadow-none dark:hover:bg-white/[0.06]"
            }
          `}
        >
          <Accordion.Header>
            <Accordion.Trigger className="flex items-center justify-between w-full px-6 py-5 text-left sm:px-8">
              <span className="text-base font-medium text-textColorWhite dark:text-white/90 md:text-lg lg:text-[17px] leading-snug">
                {item.question}
              </span>
              <div className="ml-4 transition-colors duration-300 shrink-0 text-textColorWhite/50 dark:text-white/50 group-hover:text-textColorWhite dark:group-hover:text-white">
                <div
                  className={`
                    ...
                    ${
                      isOpen
                        ? "border-mainColor/30 bg-mainColor/10 rotate-0 dark:border-white/20 dark:bg-white/10"
...
const Sidebar = memo(({ isVisible }) => {
  return (
    <div
      className={`
        h-auto rounded-3xl border p-8
        transition-all duration-700 ease-out
        border-mainColor/20 bg-white shadow-md
    ...
    >
      ...
      <p
        className={`
          mb-8 text-[15px] leading-relaxed text-textColorWhite/70 dark:text-white/60
          transition-all duration-500 ease-out
          ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}
        `}
...
const FAQAccordion = () => {
  ...
  return (
    <div
      ref={sectionRef}
      className="container min-h-screen px-4 py-12 overflow-hidden md:py-20 md:px-8 text-textColorWhite dark:text-white"
    >
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-8 lg:grid-cols-12 lg:gap-8">
          ...
          <div
            ...
            `}
          >
            <div className="rounded-3xl border border-mainColor/15 bg-white/50 dark:border-white/[0.05] dark:bg-white/[0.01] p-6 sm:p-8 shadow-sm dark:shadow-none">
              <Accordion.Root
                type="single"
                collapsible
                value={openItem}
